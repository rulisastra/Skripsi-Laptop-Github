...
#%%
plt.plot(data[1001:-1], c='b', label='test', ls=None)
plt.plot(data[0:3991], c='r', label='training', ls=None)
plt.title('DATA 8:2')
plt.xlabel('Hari-ke')
plt.ylabel('Harga')
plt.legend()
...
R = 1*np.identity(output_dim) #Kovarian Noise measurement(observasi) # tidak terlalu ngaruh
...
for i in range(epoch):
    ...
    while(index+batch_dim<=trainX.shape[0]):        
        ...
        # error di output layer -> layer 2 deltas
        layer_2_delta = layer_2_error*dtanh(layer_2)
        
        # error di hidden layer -> layer 1 delta 
        ...
        synapse_1_masuk = np.reshape(synapse_1_update,(1,-1)) # satu baris kesamping
        ...
        Mz = np.dot(Wm,sigmas)
        # Mz = Mz.sum() # Mz = np.sum(np.dot(Wm,sigmas)) # yang bener
        ...
        # Pxz = np.round(Pxz,2)
        Pxz = Pxz.sum()
        # Pxz = np.clip(Pxz.sum(),-1,1)
        
        # Kalman gain
        K = Pz * Pxz # atau inv(Pxz)
        
        P = P - np.dot(K,np.dot(Pz,K.T))

        #%% inovasi gabungan
        innovation =  ((Y-layer_2).sum()/len(layer_2_error))
        # innovation = np.subtract(Y,layer_2)
...
plt.plot(sigmas[1,:], ls=':')
plt.plot(sigmas[1+(hidden_dim*hidden_dim),:], ls=':')
...
plt.title('Hasil Prediksi Training')
plt.xlabel('data ke-')
plt.ylabel('value')
plt.legend()
plt.show()

plt.plot(trainY, c='r', marker='o', label='Nilai Seharusnya')
plt.title('Nilai Seharusnya')
plt.xlabel('data ke-')
plt.ylabel('value')
plt.legend()
plt.show()
