...
for i in range(epoch):
    ...
    while(index+batch_dim<=trainX.shape[0]):        
        ...
        layer_1 = tanh(np.dot(X,synapse_0))
...
plt.title('MLP-UKF Semua Data')
plt.legend()
plt.show()

plt.plot(testY[0:100], marker='o', label='true') #testY[0:50] buat plotting coba liat di catatan
plt.plot(y_pred[0:100], marker='o', label='prediction')
plt.title('HASIL UJI dengan metode MLP-UKF 100 data awal')
plt.xlabel('Data ke-')
plt.ylabel('Harga')
plt.legend()
plt.show()
