# sesuai paper de lima, tapi ngga berhasail karena omeganya terlalu besar
...
for i in range(epoch):
    ...
    while(index+batch_dim<=trainX.shape[0]):        
        ...
        # inisialisasi
        X_ = masuk # myu atau X_ atau W_cap_0 di delima 
        W_error = np.subtract(np.reshape(w_concat,(1,-1)),X_)
        Pp_0 = np.dot(layer_2_error,np.dot(W_error,W_error.T))
        beta = 2.
        kappa = 0

        n = X_.size # julier versi masalah 'dimension of problem'
        lambda_ = alpha**2 * (n + kappa) - n # lambda_ = 0.001
        # omega = 0.5/(n+lambda_) # de lima tapi jadi makin besar 
