...
synapse_h = 2*np.random.random((output_dim,hidden_dim)) - 1 # dengan interval [-1,1]
...
for i in range(epoch):
    ...
    index = 0
    layer_2_value = []
    prev_hidden_layer = np.full((batch_dim,hidden_dim),0) 
    future_layer_1_deltas = np.zeros(hidden_dim) # context layer (sebelumnya)
    while(index+batch_dim<=trainX.shape[0]):        
        ...
        layer_1 = tanh(np.dot(X,synapse_0)) # + np.dot(prev_hidden_layer,synapse_h))
        ...
        layer_1_delta = (np.dot(future_layer_1_deltas,synapse_h.T) + np.dot(layer_2_delta,synapse_1.T)) * dtanh(layer_1)
    
        # calculate weight update
        synapse_1_update = np.dot(np.atleast_2d(layer_1).T,(layer_2_delta))
        synapse_h_update = np.dot(np.atleast_2d(prev_hidden_layer).T,(layer_1_delta))
        ...
        n = X_p.size # julier versi masalah 'dimension of problem'
        # lambda_ = 0.001
...
# =============================================================================
        ...
        # maka....
        for k in range(n):
            sigmas[k+1] = np.subtract(X_p, -U[k])
# =============================================================================
#             sigmas[n+k+1] = np.subtract(X_p, U[k])
...
# prev_hidden_layer_p = np.full((batch_predict,hidden_dim),0) # return full array yg isinya (0) sebesar dimensi [batch_predict x hidden_dim]
# y_pred = [] # hasil akhir Y prediksi
# index = 0
# while(index+batch_predict<=testX.shape[0]):
#     X = testX[index:index+batch_predict,:]
#     layer_1p = tanh(np.dot(X,synapse_0)+np.dot(prev_hidden_layer_p,synapse_h))
#     layer_2p = tanh(np.dot(layer_1p,synapse_1))
#     y_pred.append(layer_2p)
#     prev_hidden_layer_p = layer_1p
