...
for i in range(epoch):
    ...
    while(index+batch_dim<=trainX.shape[0]):        
        ...
        n = w_concatt.size # julier versi masalah 'dimension of problem'
        
        #%% SIGMA POINTS around mean
        mean = np.sum(w_concatt) / n # mean secara keseluruhan
...
# =============================================================================
        ...
        # maka....
        for k in range(n):
            s_ = np.reshape(w_concatt,(1,-1))
            s_n = np.subtract(w_concatt, -U)
            s_n2 = np.subtract(w_concatt, U)
